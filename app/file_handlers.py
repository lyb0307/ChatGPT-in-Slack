...
def extract_pdf_content(
    file_bytes: bytes, filename: str, logger: logging.Logger
) -> Optional[str]:
    ...
    try:
        ...
        for page_num, page in enumerate(pdf_reader.pages):
            try:
                ...
                logger.warning(
                    f"Failed to extract text from page {page_num + 1} of {filename}: {e}"
                )
                continue
    ...
    try:
        ...
        if filename.lower().endswith(".docx"):
            try:
                ...
                logger.debug(
                    f"Failed to read .docx with python-docx, trying docx2txt: {e}"
                )
        ...
        if filename.lower().endswith(".doc"):
            ...
            try:
                ...
                for encoding in ["utf-8", "latin-1", "cp1252", "utf-16"]:
                    try:
                        ...
                        if text and len(text) > 10:  # Ensure we got meaningful content
                            logger.debug(
                                f"Successfully decoded .doc file with {encoding} encoding"
                            )
                            ...
                            return format_file_content(
                                text, filename, f"Word Document (Legacy, {encoding})"
                            )
                ...
                if not text:
                    logger.warning(
                        f"Could not extract text from legacy .doc file: {filename}. File may be in binary format."
                    )
                    return format_file_content(
            ...
            try:
                ...
                try:
                    ...
                    if text and text.strip():
                        ...
                        logger.warning(
                            f"No text content extracted from Word document: {filename}"
                        )
                        return None
                except Exception as e:
...
def format_file_content(
    text: str, filename: str, file_type: str, max_chars: int = 50000
) -> str:
